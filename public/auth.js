...
(function () {
  ...
  async function getServices() {
    ...
    servicesPromise = Promise.all([
      ...
      // Complete full-page Google redirects so the app feels like a normal Google sign-in flow.
      authModule.getRedirectResult(auth).then(function (credential) {
        if (credential && credential.user) return syncProfile(credential.user);
        return null;
      }).catch(function () {});
      cloudReady = true;
      return services;
  ...
  async function signInGoogle() {
    ...
    try {
      ...
      // Use redirect instead of popup. Users leave Fracture and see the real Google account screen,
      // rather than a suspicious small Firebase helper popup.
  ...
  function ensureAuthModal() {
    ...
    modal.innerHTML = ''
      ...
      + '<div class="label">Secure Google sign in</div>'
      + '<h2 id="authModalTitle">Sign in with a trusted account.</h2>'
      + '<p id="authModalMessage">Save drafts, keep report history, and return to your work across sessions.</p>'
      + '<p class="auth-trust-note">Google sign-in opens as a full-page redirect. If your Firebase project still shows a firebaseapp.com helper page for a moment, use a Firebase custom auth domain to brand that step.</p>'
      + '<label class="auth-remember"><input id="authRememberDevice" type="checkbox" checked /> <span>Remember me on this device</span></label>'
      + '<button class="btn-primary auth-wide" id="authModalGoogle" type="button">Continue at accounts.google.com</button>'
    ...
    googleBtn.addEventListener('click', function () {
      ...
      setText('authModalStatus', 'Redirecting to Google...');
  ...
  function bindSettingsActions() {
    ...
    if (googleBtn) {
      googleBtn.addEventListener('click', function () {
        setText('settingsMessage', 'Redirecting to Google...');
        runSettingsAction(googleBtn, 'Redirecting...', async function () {
