...
function readableAuditSections(audit, mode = "not-chosen") {
...
async function streamReadableAudit(res, audit, mode = "not-chosen") {
...
export async function handleAnalyze(req, res) {
  ...
  if (isTooThinForAudit(essay)) {
    ...
    return await finish(res, buildTooThinAudit(essay), false, mode);
  }

  if (!process.env.OPENROUTER_API_KEY) {
    return await finish(res, buildServiceFallbackAudit(essay, "OPENROUTER_API_KEY is not configured"), true, mode);
  }
  ...
  try {
    ...
    return await finish(res, buildServiceFallbackAudit(essay, `Failed to reach OpenRouter: ${err?.message || String(err)}`), true, mode);
  }

  writeProgress(res, 18, "Reading the live model stream");
  let rawText = "";
  let lastProgress = 18;
  const activeMessage = mode === "speech"
  ...
  try {
    ...
    return await finish(res, buildServiceFallbackAudit(essay, err?.message || String(err)), true, mode);
  } finally {
    clearInterval(heartbeat);
  }
  ...
  return await finish(res, audit, recovered, mode);
}
